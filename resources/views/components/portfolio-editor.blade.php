@props([
    'form',            // what PortfolioContent::formFor() returned
    'slug',
    'action',          // where to submit
    'cancel',          // where "Cancel" goes back to
    'submitLabel',
    'hasPhoto' => false,
    'photoUrl' => null,
    // "Your words" when it is your own page, "Their words" when an admin is
...
@php
    $labels = [
        ...
        'speech' => $speechLabel ?? __('Their words'),
    ];
    $long = ['tagline', 'speech'];
...
<form method="POST" action="{{ $action }}" enctype="multipart/form-data" class="space-y-5"
    ...
    {{-- the portrait --}}
    <div class="card p-6">
        <h3 class="font-serif text-lg">{{ __('Photograph') }}</h3>
        <p class="text-sm mb-4 measure" style="color: var(--text-mid)">
            {{ __('The portrait at the top of the page. This one is for the portfolio only — it does not change the photo on the family tree.') }}
        </p>

        <div class="flex flex-wrap items-start gap-5">
            <div class="hairline overflow-hidden shrink-0"
                 style="width: 8rem; height: 10rem; border-radius: var(--radius-control); background: var(--ink-900)">
                @if ($hasPhoto)
                    <img src="{{ $photoUrl }}" alt="" class="w-full h-full" style="object-fit: cover">
                @else
                    <div class="w-full h-full grid place-items-center text-center p-2">
                        <span class="text-xs" style="color: var(--text-low)">{{ __('No photo yet') }}</span>
                    </div>
                @endif
            </div>

            <div class="flex-1" style="min-width: 14rem">
                <x-input-label for="photo" :value="__('Choose a picture')" />
                ...
                @if ($hasPhoto)
                    <label class="flex items-center gap-2 mt-3 cursor-pointer text-sm">
                        <input type="checkbox" name="remove_photo" value="1">
                        {{ __('Remove the current photo') }}
                    </label>
                @endif
            </div>
        </div>
            ...
            <div class="card p-6">
                ...
                <p class="text-sm mb-4" style="color: var(--text-mid)">
                    {{ __('The positions listed on the page, in the order they should be read.') }}
                </p>

                <template x-if="!lists.roles.length">
                    <p class="text-sm py-2" style="color: var(--text-low)">{{ __('Nothing listed — this part of the page will be empty.') }}</p>
                </template>
            ...
            <div class="card p-6">
                ...
                <p class="text-sm mb-4" style="color: var(--text-mid)">{{ __('The bar on the page is drawn from the number.') }}</p>
            ...
            {{-- the layout --}}
            <div class="card p-6">
                <h3 class="font-serif text-lg">{{ __('The order of the page') }}</h3>
                <p class="text-sm mb-4 measure" style="color: var(--text-mid)">
                    {{ __('Move a part up or down to change where it appears, or untick it to leave it off the page altogether.') }}
                </p>
    ...
    <div class="flex flex-wrap justify-end gap-2">
        <a href="{{ $cancel }}" class="btn btn-secondary">{{ __('Cancel') }}</a>
